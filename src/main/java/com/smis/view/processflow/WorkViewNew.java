...
@Route(value = "mlaschemesnew", layout = MainLayout.class)
...
public class WorkViewNew extends VerticalLayout {
	...
	NewService service;
	Grid<WorkNew> grid = new Grid<>(WorkNew.class);
	...
	WorkFormNew workform;
	boolean isAdmin;
	boolean isUser;
	@Autowired
	private Audit audit;
	public WorkViewNew(NewService service) {
...
//		headerRow.getCell(yearColumn).setComponent(year);
		...
		GridContextMenu<WorkNew> contextMenu = new GridContextMenu<>(grid);

		// Add a menu item for viewing installments
		contextMenu.addItem("View Installments", event -> {
		    Optional<WorkNew> selectedWork = event.getItem();
	...
	private void showInstallmentsDialog(WorkNew work) { // Create a dialog
		Dialog dialog = new Dialog();
		dialog.setHeaderTitle(work.getWorkCode()+"-"+work.getWorkName());
		Grid<InstallmentNew> installmentGrid = new Grid<>(InstallmentNew.class, false);
		DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
		installmentGrid.addColumn(InstallmentNew::getInstallmentNo).setHeader("Installment Number").setResizable(true);
		installmentGrid.addColumn(InstallmentNew::getInstallmentAmount).setHeader("Amount Released").setResizable(true);
		...
		installmentGrid.addColumn(InstallmentNew::getInstallmentLetter).setHeader("Letter No.").setResizable(true);
		installmentGrid.addColumn(InstallmentNew::getUcLetter).setHeader("UC Letter No").setResizable(true);
		...
		installmentGrid.addColumn(installment->
			installment.getUcDate()!=null?
					installment.getUcDate().format(dateFormatter):"").setHeader("UC. Date").setResizable(true).setSortable(true)
						.setAutoWidth(true);
		installmentGrid.addColumn(InstallmentNew::getEnteredBy).setHeader("Entered By").setResizable(true);
		...
		List<InstallmentNew> installments = service.getInstallments(work);
	...
	public void updateGrid() {
		...
		grid.setItems(service.getWorksByUser());
	}
	...
	public void configureForm() {
		workform = new WorkFormNew(service);
		workform.setWidth("40%");
		workform.addListener(WorkFormNew.SaveEvent.class, this::saveWork);
		workform.addListener(WorkFormNew.DeleteEvent.class, this::deleteWork);
		workform.addListener(WorkFormNew.CloseEvent.class, e -> closeEditor());
	}

	public void saveWork(WorkFormNew.SaveEvent event) {
	...
	public void deleteWork(WorkFormNew.DeleteEvent event) {
	...
	private void addWork() {
		...
		editWork(new WorkNew());

	}

	private void editWork(WorkNew work) {
		try {
